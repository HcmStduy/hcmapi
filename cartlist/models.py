...
class CartModel(YGBaseModel):
    user = models.ForeignKey('user.AppUser',
...
class OrderlistModel(YGBaseModel):
    user = models.ForeignKey('user.AppUser',
                             on_delete=models.CASCADE,
                             verbose_name='所属用户',
                             related_name='order')
    order_time = models.CharField(max_length=50, verbose_name='下单时间')
    order_statues = models.IntegerField(choices=((0, '待支付'),
                                                (1, '已支付'),
                                                (2, '已取消'),
                                                (3, '待发货'),
                                                (4, '已发货'),
...
class OrderGoods(YGBaseModel):
    order = models.ForeignKey(OrderlistModel,
                              on_delete=models.CASCADE,
                              verbose_name='所属订单',
                              related_name='goods')
    goods = models.ForeignKey('goods.GoodsModel',
